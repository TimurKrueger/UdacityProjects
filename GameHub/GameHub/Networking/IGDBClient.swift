...
class IGDBClient {
    ...
    func getGames(completionHandler: @escaping (_ success: Bool, _ errorString: String?, _ games: Any) -> Void){
        ...
        let task = session.dataTask(with: requestHeader as URLRequest) { (data, response, error) in
            ...
            do {
                ...
                let responseObject = try decoder.decode(IGDBData.self, from: data)
              
                DispatchQueue.main.async {
                    completionHandler(true, nil, responseObject)
                }
            } catch {
                completionHandler(false, error.localizedDescription, nil)
                return
